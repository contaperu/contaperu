"""Driver open-accounting: el asiento en el propio estándar, para los ERP que vienen.
...
el núcleo (`tests/test_driver_open_accounting.py`). Lo que un ERP necesita además de las líneas —qué tramo es de qué
...
NOMBRE = "open_accounting"
# Un formato neutral para integrar: el grupo ERP (`drivers.contrato.GRUPOS`).
CANAL = "intercambio"
VOCABULARIO = "neutral"
FORMATOS = {"compra": "open_accounting_json", "venta": "open_accounting_json"}
...
def nombre(libro: Libro, opciones: OpcionesArchivo = OPCIONES) -> str:
    return f"open_accounting_{libro.ruc}_{libro.periodo}_{libro.tipo}{opciones.extension}"
