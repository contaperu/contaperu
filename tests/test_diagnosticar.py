...
diagnosticar = por_la_fachada(op.diagnosticar)
...
def test_los_errores_bloquean_y_van_por_serie_numero():
    d = diagnosticar(doc(FACTURA, dict(FACTURA, numero="872", igv="99")))    # IGV que no cuadra
...
def test_los_avisos_no_bloquean_pero_se_ven():
    d = diagnosticar(doc(dict(FACTURA, fecha_emision="2025-07-20")))    # 13 meses: fuera del plazo de anotación
    ...
    d = diagnosticar(doc(dict(FACTURA, fecha_emision="2026-07-20")))
...
def test_cada_faltante_se_describe_sin_lanzar(cambio, clave, motivo):
    ...
    with pytest.raises(Exception):
        exportar(doc(dict(FACTURA, **cambio)), "concar")
...
def test_la_detraccion_espera_su_constancia_hasta_que_se_pague():
    ...
    d = diagnosticar(doc(provisional, pagada))
    ...
    d2 = diagnosticar(doc(dict(FACTURA, detraccion={"codigo": "000", "porcentaje": 3})))
...
def test_el_golden_de_compras_se_diagnostica_con_la_cuenta_del_ruc():
    ...
    sin = diagnosticar(datos)
    assert sin["listo_para_exportar"] is False and len(sin["faltantes"]["sin_cuenta"]) == 3
    con = diagnosticar(datos, {"cuentas": {"gasto": "659999"}, "usa_centros_costo": False})
...
def test_lo_que_falta_dice_a_quien_pedirselo():
    ...
    d = diagnosticar(doc(dict(FACTURA, igv="99", total="4299"),                    # el IGV no es el 18 %
                            ...
                            dict(FACTURA, tipo_cp="13", serie="", numero="77")))    # sin sigla
