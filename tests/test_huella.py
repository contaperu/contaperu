...
def lineas(**cambios):
    d = op.generar_asiento(doc(dict(FACTURA, **cambios)))
...
def test_el_correlativo_no_entra():
    ...
    assert op.generar_asiento(doc(FACTURA), correlativos={"11": 500})["_asiento"]["huella"] == HUELLA_FACTURA
...
def test_es_del_asiento_y_no_del_archivo():
    ...
    d = doc(FACTURA)
    concar = op.exportar(d, "concar")
    csv = op.exportar(d, "csv")
...
def test_el_sire_no_lleva_huella():
    ...
    e = op.exportar(doc(FACTURA), "sire")["_exportacion"]
...
def test_la_fecha_la_pone_quien_llama():
    ...
    assert "fecha" not in op.exportar(doc(FACTURA), "concar")["_exportacion"]
    assert op.exportar(doc(FACTURA), "concar", fecha="2026-09-11")["_exportacion"]["fecha"] == "2026-09-11"
    with pytest.raises(op.DocumentoInvalido, match="AAAA-MM-DD"):
        op.exportar(doc(FACTURA), "concar", fecha="11/09/2026")
